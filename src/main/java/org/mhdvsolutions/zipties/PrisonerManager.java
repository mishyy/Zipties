...
public final class PrisonerManager implements ZiptiesApi {
    ...
    public ItemStack getZiptieItem() {
        ...
        String type = configuration.getString("items.zipties.type", Material.GOLD_HOE.name());
        ...
        meta.setLore(Msg.color(configuration.getStringList("items.zipties.lore")));
    ...
    public ItemStack getCuttersItem() {
        FileConfiguration configuration = plugin.getConfig();
        String type = configuration.getString("items.cutters.type", Material.GOLD_HOE.name());
        int data = configuration.getInt("items.cutters.data");

        ItemStack stack = new ItemStack(Material.valueOf(type), 1, (short) data);
        ItemMeta meta = stack.getItemMeta();
        meta.setDisplayName(Msg.color(configuration.getString("items.cutters.name")));
        meta.setLore(Msg.color(configuration.getStringList("items.cutters.lore")));
        meta.addItemFlags(ItemFlag.HIDE_UNBREAKABLE, ItemFlag.HIDE_ATTRIBUTES);
        meta.setUnbreakable(true);
        stack.setItemMeta(meta);
        return stack;
    ...
    public void restrain(Player restrainer, Player prisoner) {
        ...
        String name = plugin.getConfig().getString("sounds.restrained.name");
        float volume = plugin.getConfig().getInt("sounds.restrained.volume");
        float pitch = plugin.getConfig().getInt("sounds.restrained.pitch");
        prisoner.getWorld().playSound(prisoner.getEyeLocation(), name, volume, pitch);
    }
    ...
    public void release(Player player, ReleaseType type) {
        ...
        String id = escaped ? "escaped" : "released", name = plugin.getConfig().getString("sounds." + id + ".name");
        float volume = plugin.getConfig().getInt("sounds." + id + ".volume");
        float pitch = plugin.getConfig().getInt("sounds." + id + ".pitch");
        player.getWorld().playSound(player.getEyeLocation(), name, volume, pitch);
    }
